...
document.getElementById("registration-form").addEventListener("submit", function(event) {
  ...
  if (runnerAge > 18 && earlyRegistration) {
    ...
    raceDetailsDiv.textContent = "You will race at 9:30am with race number: " + raceNumber;
  } else if (runnerAge > 18 && !earlyRegistration) {
    // Displays race information for late-registered adults.
    raceDetailsDiv.textContent = "Late adults run at 11:00 am with race number: " + raceNumber;
  } else if (runnerAge <= 18) {
    // Displays race information for youth registrants.
    raceDetailsDiv.textContent = "Youth registrants run at 12:30 pm (regardless of registration)";
  }
});
